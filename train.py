...
def save_model(args, model, steps):
    model_to_save = model.module if hasattr(model, 'module') else model
    # model_checkpoint = os.path.join(args.output_dir, "%s_%scheckpoint.bin" % (args.name, str(steps)))
...
def setup(args):
    ...
    # config = CONFIGS[args.model__type]
...
def train(args, model):
    ...
    # Train!
    wandb.init(project="FFVT-final-project", entity="ytliu", config=args)
    ...
    while True:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if global_step % args.eval_every == 0 and args.local_rank in [-1, 0]:
                    val_acc, val_loss = valid(args, model, writer, test_loader, global_step)
                    ...
                    # if global_step >= 1000 and global_step % 100 == 0:
                    #     save_model(args, model, global_step)
                    logger.info("best accuracy so far: %f" % best_acc)
                    logger.info("best accuracy in step: %f" % best_step)
                    model.train()

                    wandb.log({'steps': global_step, 'val_acc': val_acc, 'val_loss': val_loss})
        ...
        if global_step % t_total == 0:
            wandb.log({'train_accuracy': train_accuracy,
                      'best_step': best_step, 'best_acc': best_acc})
            break
...
def main():
    ...
    args.output_dir = '{}/{}'.format(args.output_dir, args.name)
